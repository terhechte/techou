...
pub fn make_similarity(for_documents: &mut Vec<Document>, amount: usize) {
    ...
    let mut sims: Vec<Option<Vec<SimilarDocument>>> = for_documents.iter().map(|document| {
        Some(documents_by_similarity(document, &for_documents
                                , amount))
    }).collect();
...
pub fn documents_by_similarity<'a, D: AsRef<Document>>(to_document: &'a Document, in_documents: &'a [D], nr: usize)
    -> Vec<SimilarDocument> {
    ...
    let mut sorted: Vec<SimilarDocument> = in_documents.iter().filter_map(|item| {
        let item = item.as_ref();
        if item.identifier == to_document.identifier { return None }
        let titlen = normalized_damerau_levenshtein(&item.info.title, &to_document.info.title);
        let descn = normalized_damerau_levenshtein(&item.info.description, &to_document.info.description);
        let tag_intersection = HashSet::from_iter(item.info.tags.iter()).intersection(&items).count();
...
mod tests {
    ...
    fn test_relevance_hard() {
        use crate::document_operations::documents_by_similarity;
        let t1 = self::make_doc("1", "How to use pattern matching",
                                "pattern matching can be used to simplify code", &["rust", "match"]);
        let t2 = self::make_doc("2", "Rust Best Practices",
                                "these are some best practices, like pattern matching, to help you improve", &["practice", "tips", "pattern"]);
        let t3 = self::make_doc("3", "Compiler issues",
                                "How to figure out what the borrow checker wants", &["borrow checker", "help"]);
        let docs = [t2, t3];
        let d = documents_by_similarity(&t1, &docs, 2);
        assert_eq!(d[0].identifier, "2");
        assert_eq!(d[1].identifier, "3");
    ...
    fn make_doc(iden: &'static str, title: &'static str, desc: &'static str, tags: &'static [&'static str]) -> Document {
